...
class gestorNotas
{
  constructor(filename = 'notas.json'){
    this.filename = filename;
    this.notes = [];
    
    //crea la nota si no existe el archivo
    try{
      if( !fs.existsSync(this.filename) ){
        fs.writeFileSync(this.filename, JSON.stringify( [], null, 2]  ));
        console.log(`se han creado nuevos archivos ${this.filename}`);
      } else{
        this.loadNotes();
      }
    } catch(error){
      console.error("error al inicializar el objeto gestorNotas", error);
    }
  }
...
function agregarNota(titulo, contenido) 
{
  ...
  fs.writeFileSync(filePath, notas);
...
function listarNotas() 
{
  if (fs.existsSync(filePath)) {
    ...
    const rawData = fs.readFileSync( filePath, 'utf8', (error, datos) => {
      console.log(datos);
    } );
  } else {
